...
def read_mat(env, path, size=None):
    """read image by cv2 or from lmdb
    return: Numpy float32, HWC, BGR, [0,1]"""
    ...
    event_xyz = torch.concat((event_xyz, event_xyz), axis=1)
...
def colorjitter(img_GT):
    ...
    img_GT = cv2PIL((img_GT * 255).astype(np.uint8))

    # img_GT = torchvision.transforms.ColorJitter(
    #     brightness=(0.5, 1), contrast=(0.5, 1), saturation=(0.5, 1), hue=0)(img_GT)
    img_GT = torchvision.transforms.ColorJitter(
        brightness=(0.5, 1), contrast=0, saturation=0, hue=0
    )(img_GT)
    img_GT = PIL2cv(img_GT)
...
def add_noise(image, prob=0.04, mean=0, var=25, use_evsmask=False):
    noisy_image = np.copy(image)
    if not use_evsmask:
        for i in range(3):
            salt_mask = np.random.rand(*image.shape[:2]) < prob
            noisy_image[salt_mask, i] = np.random.normal(
                mean, var, np.count_nonzero(salt_mask)
            )
    else:
        salt_mask = np.random.rand(*image.shape[:2]) < 1 / 32
...
def augData(
    ...
    if use_gaussian:
        prob = np.random.uniform(0.005, 0.009)
        img = add_noise(img, prob)
        # cv2.imwrite("/root/gtdata.png", img[:, :, [2, 1, 0]] * 255)
    else:
        noise = noise[rnd_h : rnd_h + GT_size, rnd_w : rnd_w + GT_size, :]
        # print(noise.shape)
        ...
        if noise_delta:
            ...
            img = img - noise_delta / 255
            img = np.clip(img, 0, 1)
            # print("A", img.shape)
            # cv2.imwrite("/root/delta.png", img[:, :, [2, 1, 0]] * 255)

        mask = np.where(noise == 0, 1, 0)
        img = img * mask + noise / 255
        # cv2.imwrite("/root/mix.png", img[:, :, [2, 1, 0]] * 255)

    if use_evsmask:
        img = add_noise(img, use_evsmask=True)
        # cv2.imwrite("/root/gtdata.png", img[:, :, [2, 1, 0]] * 255)

    # print(noise/255)
    img = evsQuadBayerSampler(img, use_evsavg)
